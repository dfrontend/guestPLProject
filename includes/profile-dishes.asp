<section class="module-dishes">
<div class="module-title"><i class="icon-gp-cutlery v-align-mid"></i> Dishes</div>
<div class="module-listing dishes">
<!-- item -->
	<div class="list-item">
		<div class="row">
			<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4 thumb-pic">
				<img src="../img/thumb/photo-7.jpg" alt="" title="">
			</div>
			<div class="col-lg-10 col-md-10 col-sm-10 col-xs-8">
				<div class="row">
					<div class="col-lg-9 col-md-9 col-sm-9 col-xs-9">
						<h3>Tuscan Kale Salad</h3>
					</div>
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 text-right">
						<div class="price">$13</div>
					</div>
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<small>Midnight Moon / Currants / Market Veggies</small>
					</div>
					<div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
						<div class="review">
							<i class="icon-gp-star-solid font-14 v-align-mid"></i> <span>3 reviews</span> <i class="icon-gp-angle-right font-14 v-align-mid"></i>
						</div>
					</div>
					<div class="col-lg-3 col-md-3 col-sm-3 pull-right text-right col-xs-12">
						<button type="button" class="btn btn-add-order rippler rippler-default">Add to Order</button>
					</div>
				</div>
			</div>
		</div>
	</div>
<!-- item -->
<!-- item -->
	<div class="list-item">
		<div class="row">
			<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4 thumb-pic">
				<img src="../img/thumb/photo-8.jpg" alt="" title="">
			</div>
			<div class="col-lg-10 col-md-10 col-sm-10 col-xs-8">
				<div class="row">
					<div class="col-lg-9 col-md-9 col-sm-9 col-xs-9">
						<h3>Tuscan Kale Salad</h3>
					</div>
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 text-right">
						<div class="price">$13</div>
					</div>
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<small>Midnight Moon / Currants / Market Veggies</small>
					</div>
					<div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
						<div class="review">
							<i class="icon-gp-star-solid font-14 v-align-mid"></i> <span>3 reviews</span> <i class="icon-gp-angle-right font-14 v-align-mid"></i>
						</div>
					</div>
					<div class="col-lg-3 col-md-3 col-sm-3 pull-right text-right col-xs-12">
						<button type="button" class="btn btn-add-order rippler rippler-default">Add to Order</button>
					</div>
				</div>
			</div>
		</div>
	</div>
<!-- item -->
<!-- item -->
	<div class="list-item">
		<div class="row">
			<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4 thumb-pic">
				<img src="../img/thumb/photo-9.jpg" alt="" title="">
			</div>
			<div class="col-lg-10 col-md-10 col-sm-10 col-xs-8">
				<div class="row">
					<div class="col-lg-9 col-md-9 col-sm-9 col-xs-9">
						<h3>Tuscan Kale Salad</h3>
					</div>
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 text-right">
						<div class="price">$13</div>
					</div>
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<small>Midnight Moon / Currants / Market Veggies</small>
					</div>
					<div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
						<div class="review muted">
							<i class="icon-gp-star-solid font-14 v-align-mid"></i> <span>3 reviews</span> <i class="icon-gp-angle-right font-14 v-align-mid"></i>
						</div>
					</div>
					<div class="col-lg-3 col-md-3 col-sm-3 pull-right text-right col-xs-12">
						<button type="button" class="btn btn-add-order rippler rippler-default">Add to Order</button>
					</div>
				</div>
			</div>
		</div>
	</div>
<!-- item -->
<!-- item -->
	<div class="list-item">
		<div class="row">
			<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4 thumb-pic">
				<img src="../img/thumb/photo-7.jpg" alt="" title="">
			</div>
			<div class="col-lg-10 col-md-10 col-sm-10 col-xs-8">
				<div class="row">
					<div class="col-lg-9 col-md-9 col-sm-9 col-xs-9">
						<h3>Tuscan Kale Salad</h3>
					</div>
					<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 text-right">
						<div class="price">$13</div>
					</div>
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<small>Midnight Moon / Currants / Market Veggies</small>
					</div>
					<div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
						<div class="review muted">
							<i class="icon-gp-star-solid font-14 v-align-mid"></i> <span>0 reviews</span> <i class="icon-gp-angle-right font-14 v-align-mid"></i>
						</div>
					</div>
					<div class="col-lg-3 col-md-3 col-sm-3 pull-right text-right col-xs-12">
						<button type="button" class="btn btn-add-order rippler rippler-default">Add to Order</button>
					</div>
				</div>
			</div>
		</div>
	</div>
<!-- item -->
</div>
</section>