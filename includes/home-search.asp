<section class="home-search">
	<div class="text-center">
		<h1 class="clearfix">Where Would You Like to Eat?</h1>
		<small class="clearfix font-light">Authentic home cooked meals from local hosts.</small>
	</div>


	<div class="container">
		<div class="row">
			<div class="col-lg-8 col-md-8 col-lg-offset-2 col-md-offset-2">
				<div class="search-module">
					<div class="wrapper clearfix">
					<form class="form-inline">
						<div class="form-group pull-left width-50pr pos-relative">
							<i class="icon-gp-location"></i>
							<input type="text" class="form-control width-100pr" id="location" placeholder="Location">
						</div>
						<div class="pull-left width-30pr pos-relative">
							<div class="dropdown">
							<i class="icon-gp-plate-cover"></i>
								<button id="category" class="width-100pr clearfix text-left" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="btn-value display-block truncate">Category</span> 
								<span class="glyphicon glyphicon-chevron-down pull-right"></span>
								</button>

								<ul class="dropdown-menu width-100pr" aria-labelled="category">
									<li><a href="javascript:void(0);">Category 1</a></li>
									<li><a href="javascript:void(0);">Category 2</a></li>
									<li><a href="javascript:void(0);">Category long text</a></li>
								</ul>
							</div>
						</div>
						<div class="pull-left width-20pr">
							<button type="button" class="btn color-white width-100pr rippler rippler-default">Search</button>
						</div>
					</form>

					</div>


				</div>
			</div>
			<div class="clearfix"></div>
					<div class="text-center">
						<button type="button" class="btn btn-lg rippler rippler-default">How It Works</button>	
					</div>
			
		</div>
	</div>
</section>